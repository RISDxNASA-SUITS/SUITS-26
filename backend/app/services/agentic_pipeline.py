"""LLM router: telemetry Q&A from current snapshot."""
...
RouterAction = Literal["telemetry", "unknown"]
...
{"action":"telemetry"|"unknown","reason":string|null}
Rules:
...
def _parse_router(raw: str) -> tuple[RouterAction, str | None]:
    try:
    ...
    if action not in ("telemetry", "unknown"):
        return "unknown", None
...
def run_agentic_pipeline(input_text: str) -> CommandResponse:
    ...
    action, _reason = _parse_router(out.text)
